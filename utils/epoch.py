...
def train_classifier_net(model, train_dataloader, optimizer, epoch):
    model.train() 
...
def train_siamese_net(model, train_dataloader, optimizer, epoch):
    model.train()
    epoch_loss, epoch_acc = 0.0, 0.0
...
def valid_siamese_net(model, valid_dataloader, epoch):
    model.eval() 
    epoch_loss, epoch_acc = 0.0, 0.0
    with torch.no_grad():
